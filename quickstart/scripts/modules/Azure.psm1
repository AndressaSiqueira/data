...
function SetupResourceGroups {
    ...
    foreach ($resourceType in @('data','compute','ml','network', 'security')) {
        $SolutionName = $Configuration.project.alias

        # Captura o Resource Group retornado
        ...
        # Armazena o Resource Group na variável global
        $Global:ResourceGroups["$Environment-$resourceType"] = $resourceGroup.ResourceGroupName
    }

    # Captura o Resource Group para template-specs
    $templateSpecsResourceGroup = CreateOrGetResourceGroup `
        -Name "rg-$SolutionName-template-specs" `
        -Location $Configuration.project.location

    $Global:ResourceGroups["$Environment-template-specs"] = $templateSpecsResourceGroup.ResourceGroupName
}
