...
function SetupServiceConnection {
    ...
    if (!$serviceEndpointId) {
        ...
        Write-Host "Organization URI: $organizationURI"
        ...
        $organizationId = az devops organization show --organization $organizationURI --query "id" -o tsv

        # Usar o Organization ID na URL do Issuer
            $issuer = "https://vstoken.dev.azure.com/$organizationId"
        }
    
    function SetupFederatedCredential {
        [cmdletbinding()]
        param (
            [Parameter(Mandatory)] [string] $Environment,
            [Parameter(Mandatory)] [hashtable] $ServicePrincipal
    ...
    # Obter o Resource Group para o ambiente e tipo de recurso desejado
    $resourceGroupName = $Global:ResourceGroups["$Environment-data"]

    if (!$resourceGroupName) {
        throw "Resource Group for environment '$Environment' and type 'data' not found."
    }

        # Adicaoo do Identity Federation 
        az identity federated-credential create `
            --name "$serviceConnectionName-federated-credential" `
            --identity-name $ServicePrincipal.displayName `
            --resource-group $resourceGroupName `
            --issuer $issuer `
            --subject "sc://andressasiqueira02/$project/$serviceConnectionName"
    Write-Host "Federated credential added successfully to the Service Principal."
}
    
    LogInfo -Message "Granting access permission to all pipelines on the '$serviceConnectionName' service connection..."
